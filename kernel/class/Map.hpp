...
private:
    ...
    void destroy_(RBNode *);// 销毁函数，用于递归地销毁一个给定的子树，并释放其内存空间
    void leftRotate_(RBNode *);// 左旋函数，将给定节点作为旋转轴，将其右子节点提升为新的父节点，并调整相关指针和颜色
    void rightRotate_(RBNode *);// 右旋函数，将给定节点作为旋转轴，将其左子节点提升为新的父节点，并调整相关指针和颜色
    void insertFixup_(RBNode *);// 插入修复函数，用于在插入新节点后，修复红黑树的性质
    void deleteFixup_(RBNode *);// 删除修复函数，用于在删除一个节点后，修复红黑树的性质
    RBNode *search_(const Key &);// 查找函数，用于在红黑树中查找给定的数据，返回对应的节点指针，如果不存在则返回空指针
    void insert_(const Key &, const Val &);// 插入函数，用于在红黑树中插入一个新的数据，如果已经存在则不插入
    void remove_(const Key &);// 删除函数，用于在红黑树中删除一个给定的数据，如果不存在则不删除
    void transplant_(RBNode *, RBNode *);// 替换函数，用于在红黑树中用一个子树替换另一个子树，保持相关指针和颜色不变
    RBNode *minNum_(RBNode *);// 最小值函数，用于在红黑树中找到一个给定子树的最小值节点，返回其指针
    int size_(RBNode *);//递归计算节点个数
};
...
template<typename Key, typename Val>
void Map<Key, Val>::destroy_(RBNode *x) {
...
void Map<Key, Val>::remove(const Key &key) {
    remove_(key); // 调用删除函数，完成删除操作
}

template<typename Key, typename Val>
void Map<Key, Val>::leftRotate_(RBNode *x)  {
...
template<typename Key, typename Val>
void Map<Key, Val>::rightRotate_(RBNode *x) {
...
template<typename Key, typename Val>
void Map<Key, Val>::insertFixup_(RBNode *z) {
        ...
        if (z->parent == z->parent->parent->left) { // 如果新节点的父节点是其祖父节点的左子节点
            ...
            if (y != nullptr && y->color == RBNode::RED) { // 如果叔叔节点是红色，情况一
                ...
                if (z == z->parent->right) { // 如果新节点是其父节点的右子节点，情况二
                    z = z->parent; // 将新节点上移一层
                    leftRotate_(z); // 对新节点进行左旋，转化为情况三
                }
                // 如果新节点是其父节点的左子节点，情况三
                z->parent->color = RBNode::BLACK; // 将父节点染成黑色
                z->parent->parent->color = RBNode::RED; // 将祖父节点染成红色
                rightRotate_(z->parent->parent); // 对祖父节点进行右旋，完成调整
            }
            ...
            if (y != nullptr && y->color == RBNode::RED) { // 如果叔叔节点是红色，情况一
                ...
                if (z == z->parent->left) { // 如果新节点是其父节点的左子节点，情况二
                    z = z->parent; // 将新节点上移一层
                    rightRotate_(z); // 对新节点进行右旋，转化为情况三
                }
                // 如果新节点是其父节点的右子节点，情况三
                z->parent->color = RBNode::BLACK; // 将父节点染成黑色
                z->parent->parent->color = RBNode::RED; // 将祖父节点染成红色
                leftRotate_(z->parent->parent); // 对祖父节点进行左旋，完成调整
            }
        }
    }
...
template<typename Key, typename Val>
void Map<Key, Val>::deleteFixup_(RBNode *x) {
    while (x != root && x != nullptr && x->color == RBNode::BLACK) { // 如果被删除的原始位置是黑色，需要进行调整
        if (x == x->parent->left) { // 如果被删除的原始位置是其父亲的左子位置 
            ...
            if (w->color == RBNode::RED) {  // 情况一：兄弟位置是红色 
                ...
                leftRotate_(x->parent); // 对父亲位置进行左旋，转化为情况二、三或四
            ...
            if (w->left->color == RBNode::BLACK && w->right->color == RBNode::BLACK) { // 情况二：兄弟位置是黑色，且其两个子位置都是黑色
                ...
                if (w->right->color == RBNode::BLACK) { // 情况三：兄弟位置的右子位置是黑色，左子位置是红色
                    ...
                    w->color = RBNode::RED; // 将兄弟位置染成红色
                    rightRotate_(w); // 对兄弟位置进行右旋，转化为情况四
                    w = x->parent->right; // 更新兄弟位置
                }
                ...
                leftRotate_(x->parent); // 对父亲位置进行左旋，完成调整
            ...
            if (w->color == RBNode::RED) {  // 情况一：兄弟位置是红色 
                w->color = RBNode::BLACK; // 将兄弟位置染成黑色
                x->parent->color = RBNode::RED; // 将父亲位置染成红色
                rightRotate_(x->parent); // 对父亲位置进行右旋，转化为情况二、三或四
            ...
            if (w->right->color == RBNode::BLACK && w->left->color == RBNode::BLACK) { // 情况二：兄弟位置是黑色，且其两个子位置都是黑色
                ...
                rightRotate_(x->parent);  // 对父亲位进行右旋，完成调整 
...
template<typename Key, typename Val>
typename Map<Key, Val>::RBNode *Map<Key, Val>::search_(const Key &key) {
...
template<typename Key, typename Val>
void Map<Key, Val>::insert_(const Key &key, const Val &val) {
    ...
    if (y == nullptr) // 如果父节点为空，说明红黑树为空，将新节点设为根节点
        ...
        y->right = z;
    insertFixup_(z); // 调用插入修复函数，修复红黑树的性质
}

template<typename Key, typename Val>
void Map<Key, Val>::remove_(const Key &key) {
    RBNode *z = search_(key); // 调用查找函数，找到要删除的目标节点
    ...
    if (z->left == nullptr) { // 如果目标节点没有左子树，用其右子树替代它
        ...
        y = minNum_(z->right); // 找到其右子树中的最小节点，作为替代节点
        ...
        if (y->parent == z) { // 如果替代节点是目标节点的右子节点
            ...
            transplant_(y, y->right); // 用替代节点的右子树替换替代节点
            y->right = z->right; // 将目标节点的右子树接到替代节点的右边
            y->right->parent = y; // 更新右子树的父指针
        }
        transplant_(z, y); // 用替代节点替换目标节点
...
template<typename Key, typename Val>
void Map<Key, Val>::transplant_(RBNode *u, RBNode *v) {
...
template<typename Key, typename Val>
typename Map<Key, Val>::RBNode *Map<Key, Val>::minNum_(RBNode *x) {
...
bool Map<Key, Val>::contain(const Key &key) {
    RBNode *x = search_(key); // 调用查找函数，找到对应的节点指针
    return x != nullptr; // 如果节点指针不为空，说明存在，返回真，否则返回假
}

template<typename Key, typename Val>
int Map<Key, Val>::size() {
    return size_(root); // 调用递归函数，从根节点开始计算
}

template<typename Key, typename Val>
int Map<Key, Val>::size_(RBNode *x) {
    if (x == nullptr) // 如果当前节点为空，返回0
        return 0;
    else // 如果当前节点不为空，返回1加上其左右子树的节点个数之和
        return 1 + size_(x->left) + size_(x->right);
}
