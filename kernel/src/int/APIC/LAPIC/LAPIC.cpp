...
namespace LAPIC {
    ...
    void init() {
        ...
        reg(LAPIC_LVT_LINT0) = LAPIC_INTNUM_LINT0 | LAPIC_DM(LAPIC_DM_FIXED) | LAPIC_MASK(0);//LINT0
        reg(LAPIC_LVT_LINT1) = 0 | LAPIC_DM(LAPIC_DM_NMI) | LAPIC_MASK(1);//LINT1
        reg(LAPIC_LVT_ERROR) = LAPIC_INTNUM_ERROR | LAPIC_DM(LAPIC_DM_FIXED) | LAPIC_MASK(0);//Error
        reg(LAPIC_SPIVR) |= (0x100 | LAPIC_INTNUM_SPU);//软启用LAPIC, 并设置伪中断号为255
    }
}
