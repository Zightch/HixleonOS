...
void kernelMain() {
    ...
    //寻找1MB开始的DRAM内存
    unsigned short memIndex;//内存地图下标
    unsigned int memBase = -1;//内存基地址
    unsigned int memUpper = 0;//最大寻址地址
    unsigned int memSize = 0;//内存长度
    for (memIndex = 0; memIndex < mapSize; memIndex++) {
        if (memMap[memIndex].base == 0x100000) {
            memUpper = memMap[memIndex].size;
            if (memMap[memIndex].size > 0xFFF00000)
                memUpper = 0xFFF00000;
            memUpper += 0xFFFFF;
            memBase = 0x100000;
